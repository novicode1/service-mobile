...
export default {
  ...
  computed: {
    sProducts() {
      return this.limit ? this.$store.getters.sale.filter(el => el.price < this.highprice).slice(0,this.limit) : this.$store.getters.sale.filter(el => el.price < this.highprice);
    }
  }
};
</script>
